/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */
#include "SEGGER_RTT.h"
#include "RDA.h"
#include "DISPLAY.h"
#include "TEMP.h"
#include "sd_fs.h"
#include "font.h"

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

#define IMAGE_WIDTH_PIXELS 240 // All values calculated as bytes. One pixel is 2 bytes
#define IMAGE_HEIGHT_PIXELS 50
#define MAX_LOAD_SIZE IMAGE_WIDTH_PIXELS * IMAGE_HEIGHT_PIXELS

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define LED_Pin GPIO_PIN_13
#define LED_GPIO_Port GPIOC
#define SCREEN_CS_Pin GPIO_PIN_3
#define SCREEN_CS_GPIO_Port GPIOA
#define SCREEN_DS_Pin GPIO_PIN_4
#define SCREEN_DS_GPIO_Port GPIOA
#define SCREEN_CLK_Pin GPIO_PIN_5
#define SCREEN_CLK_GPIO_Port GPIOA
#define SCREEN_MISO_Pin GPIO_PIN_6
#define SCREEN_MISO_GPIO_Port GPIOA
#define SCREEN_MOSI_Pin GPIO_PIN_7
#define SCREEN_MOSI_GPIO_Port GPIOA
#define SCREEN_LED_Pin GPIO_PIN_0
#define SCREEN_LED_GPIO_Port GPIOB
#define SCREEN_RESET_Pin GPIO_PIN_1
#define SCREEN_RESET_GPIO_Port GPIOB
#define SD_CS_Pin GPIO_PIN_2
#define SD_CS_GPIO_Port GPIOB
#define SD_SCK_Pin GPIO_PIN_10
#define SD_SCK_GPIO_Port GPIOB
#define SD_MISO_Pin GPIO_PIN_14
#define SD_MISO_GPIO_Port GPIOB
#define SD_MOSI_Pin GPIO_PIN_15
#define SD_MOSI_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

typedef enum {
	RADIO = 0,
	BLUETOOTH = 1,
	ANALOG = 2,
} PLAY_MODE;

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
